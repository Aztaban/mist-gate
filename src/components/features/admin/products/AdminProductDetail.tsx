...
const AdminProductDetail = ({ product, onEdit }: AdminProductDetailProps): JSX.Element => {
  ...
  const imgUrl = (product as any).imageUrl ?? product.image;
  ...
  return (
    <>
          ...
          <div className="product-editor__grid product-editor__grid--detail">
            ...
            <aside className="product-editor__col product-editor__col--media admin-product-detail__side">
              <div className="product-editor__media admin-product-detail__media">
                {/* use imageUrl if you have it, otherwise product.image */}
      ...
      {/* REAL MODALS – overlay, independent of layout */}
      {activeModal === 'restock' && (
        <RestockModal currentStock={product.countInStock} productId={product.id} onClose={closeModal} />
      )}

      {activeModal === 'priceChange' && (
        <PriceChangeModal currentPrice={product.price} productId={product.id} onClose={closeModal} />
      )}

      {activeModal === 'image' && (
        <ProductImageUpadteModal productId={product.id} currentImage={imgUrl} onClose={closeModal} />
      )}
    </>
  );
};
