...
const Nav = forwardRef<HTMLElement, NavProps>(
  ({ isMenuOpen, closeMenu }, ref) => {
    ...
    const { isLogedIn, isAdmin, username, isEditor } = useAuth();
    ...
    return (
      <nav ref={ref} className={isMenuOpen ? 'dropdown-menu' : 'normal-menu'}>
        <ul className={isMenuOpen ? 'nav-links' : ''} onClick={handleMenuClick}>
          {isAdmin || isEditor ? adminNavListItems : userNavListItems}
          {isMenuOpen && isLogedIn ? (
            <li className="black-and-gold-text" onClick={logout}>
              Logout
            </li>
          ) : null}
        </ul>
